...
<script language="JavaScript">
	...
	function showparent(tgl,bln,thn,textid){
		...
		window.opener.document.getElementById(textid).value="01/"+bln+"/"+thn;
		<?php }else{ ?>
		// window.opener.document.getElementById(textid).value=thn+"-"+bln+"-"+tgl;
		window.opener.document.getElementById(textid).value=tgl+"/"+bln+"/"+thn;
		<?php } ?>
		window.close();
	}
</script>
<?php
	...
	if($datevalue){
		...
		$_GET['year']=substr($datevalue,6,4);
		$_GET['month']=substr($datevalue,3,2);
		$_GET['tanggal']=substr($datevalue,0,2);
